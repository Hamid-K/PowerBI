...
namespace Microsoft.Data.Edm.Csdl.Internal.CsdlSemantics
{
	...
	{
		// Token: 0x06000289 RID: 649 RVA: 0x000063F8 File Offset: 0x000045F8
		...
		// (get) Token: 0x0600028A RID: 650 RVA: 0x00006453 File Offset: 0x00004653
		...
		// (get) Token: 0x0600028B RID: 651 RVA: 0x0000645B File Offset: 0x0000465B
		...
		// (get) Token: 0x0600028C RID: 652 RVA: 0x00006463 File Offset: 0x00004663
		...
		// (get) Token: 0x0600028D RID: 653 RVA: 0x0000646B File Offset: 0x0000466B
		...
		// (get) Token: 0x0600028E RID: 654 RVA: 0x00006478 File Offset: 0x00004678
		...
		// (get) Token: 0x0600028F RID: 655 RVA: 0x0000648C File Offset: 0x0000468C
		...
		// (get) Token: 0x06000290 RID: 656 RVA: 0x000064A0 File Offset: 0x000046A0
		...
		// (get) Token: 0x06000291 RID: 657 RVA: 0x000064DC File Offset: 0x000046DC
		...
		// Token: 0x06000293 RID: 659 RVA: 0x0000650C File Offset: 0x0000470C
			...
			else
			{
				if (num == 3)
				{
					...
					if (edmEntityContainer != null)
					{
						...
						{
							IEdmFunctionParameter edmFunctionParameter2 = edmFunctionImport2.FindParameter(text4);
							if (edmFunctionParameter2 != null)
							{
								return edmFunctionParameter2;
							}
							return new UnresolvedParameter(edmFunctionImport2, text4, base.Location);
						}
					}
					string text5 = text2 + "/" + text3;
					UnresolvedFunction unresolvedFunction = new UnresolvedFunction(text5, Strings.Bad_UnresolvedFunction(text5), base.Location);
					return new UnresolvedParameter(unresolvedFunction, text4, base.Location);
				}
		...
		// Token: 0x06000294 RID: 660 RVA: 0x000067D4 File Offset: 0x000049D4
		private T FindParameterizedFunction<T>(string parameterizedName, Func<string, IEnumerable<T>> findFunctions, Func<IEnumerable<T>, T> ambiguityCreator) where T : class, IEdmFunctionBase
		{
			...
			IEnumerable<T> enumerable = Enumerable.Cast<T>(this.FindParameterizedFunctionFromList(Enumerable.Cast<IEdmFunctionBase>(findFunctions.Invoke(text)), array));
			if (Enumerable.Count<T>(enumerable) == 0)
			{
				return default(T);
			}
			if (Enumerable.Count<T>(enumerable) == 1)
			{
				return Enumerable.First<T>(enumerable);
			}
			return ambiguityCreator.Invoke(enumerable);
		}

		// Token: 0x06000295 RID: 661 RVA: 0x00006880 File Offset: 0x00004A80
		private IEdmFunctionImport CreateAmbiguousFunctionImport(IEnumerable<IEdmFunctionImport> functions)
		{
			...
			enumerator.MoveNext();
			IEdmFunctionImport edmFunctionImport2 = enumerator.Current;
			AmbiguousFunctionImportBinding ambiguousFunctionImportBinding = new AmbiguousFunctionImportBinding(edmFunctionImport, edmFunctionImport2);
			while (enumerator.MoveNext())
			{
				IEdmFunctionImport edmFunctionImport3 = enumerator.Current;
				ambiguousFunctionImportBinding.AddBinding(edmFunctionImport3);
			}
			return ambiguousFunctionImportBinding;
		}

		// Token: 0x06000296 RID: 662 RVA: 0x000068D0 File Offset: 0x00004AD0
		private IEdmFunction CreateAmbiguousFunction(IEnumerable<IEdmFunction> functions)
		{
			...
			enumerator.MoveNext();
			IEdmFunction edmFunction2 = enumerator.Current;
			AmbiguousFunctionBinding ambiguousFunctionBinding = new AmbiguousFunctionBinding(edmFunction, edmFunction2);
			while (enumerator.MoveNext())
			{
				IEdmFunction edmFunction3 = enumerator.Current;
				ambiguousFunctionBinding.AddBinding(edmFunction3);
			}
			return ambiguousFunctionBinding;
		}

		// Token: 0x06000297 RID: 663 RVA: 0x00006920 File Offset: 0x00004B20
		private IEnumerable<IEdmFunctionBase> FindParameterizedFunctionFromList(IEnumerable<IEdmFunctionBase> functions, string[] parameters)
		{
			...
			{
				if (Enumerable.Count<IEdmFunctionParameter>(edmFunctionBase.Parameters) == Enumerable.Count<string>(parameters))
				{
					bool flag = true;
