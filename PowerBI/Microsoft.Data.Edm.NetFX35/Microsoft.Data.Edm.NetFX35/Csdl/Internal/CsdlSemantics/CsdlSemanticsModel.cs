...
namespace Microsoft.Data.Edm.Csdl.Internal.CsdlSemantics
{
	...
	{
		// Token: 0x06000822 RID: 2082 RVA: 0x00015FD4 File Offset: 0x000141D4
		...
		// (get) Token: 0x06000823 RID: 2083 RVA: 0x000164DC File Offset: 0x000146DC
		...
		// (get) Token: 0x06000824 RID: 2084 RVA: 0x000164FC File Offset: 0x000146FC
			...
			get
			{
				...
				foreach (CsdlSemanticsSchema csdlSemanticsSchema in this.schemata)
				{
					foreach (CsdlAnnotations csdlAnnotations in ((CsdlSchema)csdlSemanticsSchema.Element).OutOfLineAnnotations)
					{
						...
						foreach (CsdlVocabularyAnnotationBase csdlVocabularyAnnotationBase in csdlAnnotations.Annotations)
						{
							IEdmVocabularyAnnotation edmVocabularyAnnotation = this.WrapVocabularyAnnotation(csdlVocabularyAnnotationBase, csdlSemanticsSchema, null, csdlSemanticsAnnotations, csdlAnnotations.Qualifier);
		...
		// (get) Token: 0x06000825 RID: 2085 RVA: 0x00016838 File Offset: 0x00014A38
		public IEnumerable<EdmError> Errors
		{
			get
			{
				...
				foreach (IEdmAssociation edmAssociation in this.associationDictionary.Values)
				{
					...
					if (edmAssociation.IsBad())
					{
						...
						if (ambiguousAssociationBinding != null)
						{
							...
							using (IEnumerator<IEdmAssociation> enumerator2 = ambiguousAssociationBinding.Bindings.GetEnumerator())
							{
								...
								{
									IEdmAssociation edmAssociation2 = enumerator2.Current;
									if (flag)
									{
										flag = false;
									}
									...
									{
										list.Add(new EdmError(edmAssociation2.Location(), EdmErrorCode.AlreadyDefined, Strings.EdmModel_Validator_Semantic_SchemaElementNameAlreadyDefined(text)));
									}
								}
								continue;
							}
					...
					{
						if (base.FindDeclaredType(text) != null || base.FindDeclaredValueTerm(text) != null || Enumerable.Count<IEdmFunction>(base.FindDeclaredFunctions(text)) != 0)
						{
		...
		// Token: 0x06000826 RID: 2086 RVA: 0x00016A14 File Offset: 0x00014C14
		public IEdmAssociation FindAssociation(string qualifiedName)
		{
			IEdmAssociation edmAssociation;
			this.associationDictionary.TryGetValue(qualifiedName, ref edmAssociation);
			return edmAssociation;
		}

		// Token: 0x06000827 RID: 2087 RVA: 0x00016A34 File Offset: 0x00014C34
		public override IEnumerable<IEdmVocabularyAnnotation> FindDeclaredVocabularyAnnotations(IEdmVocabularyAnnotatable element)
		{
			...
			if (text != null && this.outOfLineAnnotations.TryGetValue(text, ref list))
			{
				...
				{
					foreach (CsdlVocabularyAnnotationBase csdlVocabularyAnnotationBase in csdlSemanticsAnnotations.Annotations.Annotations)
					{
						list2.Add(this.WrapVocabularyAnnotation(csdlVocabularyAnnotationBase, csdlSemanticsAnnotations.Context, null, csdlSemanticsAnnotations, csdlSemanticsAnnotations.Annotations.Qualifier));
					}
				}
		...
		// Token: 0x06000828 RID: 2088 RVA: 0x00016B50 File Offset: 0x00014D50
		public override IEnumerable<IEdmStructuredType> FindDirectlyDerivedTypes(IEdmStructuredType baseType)
		{
			...
			if (this.derivedTypeMappings.TryGetValue(((IEdmSchemaType)baseType).Name, ref list))
			{
				return Enumerable.Where<IEdmStructuredType>(list, (IEdmStructuredType t) => t.BaseType == baseType);
			}
			return Enumerable.Empty<IEdmStructuredType>();
		}

		// Token: 0x06000829 RID: 2089 RVA: 0x00016BA8 File Offset: 0x00014DA8
		...
		// Token: 0x0600082A RID: 2090 RVA: 0x00016D8C File Offset: 0x00014F8C
		...
		// Token: 0x0600082B RID: 2091 RVA: 0x00016F00 File Offset: 0x00015100
		...
		// Token: 0x0600082C RID: 2092 RVA: 0x00016F28 File Offset: 0x00015128
		internal IEnumerable<IEdmVocabularyAnnotation> WrapInlineVocabularyAnnotations(CsdlSemanticsElement element, CsdlSemanticsSchema schema)
		{
			...
			if (edmVocabularyAnnotatable != null)
			{
				...
				if (Enumerable.FirstOrDefault<CsdlVocabularyAnnotationBase>(vocabularyAnnotations) != null)
				{
					List<IEdmVocabularyAnnotation> list = new List<IEdmVocabularyAnnotation>();
		...
		// Token: 0x0600082D RID: 2093 RVA: 0x0001702C File Offset: 0x0001522C
		...
		{
			return EdmUtil.DictionaryGetOrUpdate<CsdlVocabularyAnnotationBase, CsdlSemanticsVocabularyAnnotation>(this.wrappedAnnotations, annotation, delegate(CsdlVocabularyAnnotationBase ann)
			{
				CsdlValueAnnotation csdlValueAnnotation = ann as CsdlValueAnnotation;
				...
				return new CsdlSemanticsValueAnnotation(schema, targetContext, annotationsContext, csdlValueAnnotation, qualifier);
			});
		}

		// Token: 0x0600082E RID: 2094 RVA: 0x00017084 File Offset: 0x00015284
		private void AddSchema(CsdlSchema schema)
		{
			...
			foreach (IEdmFunction edmFunction in csdlSemanticsSchema.Functions)
			{
				base.RegisterElement(edmFunction);
			}
