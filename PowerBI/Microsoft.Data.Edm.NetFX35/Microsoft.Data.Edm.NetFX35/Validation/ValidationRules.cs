...
namespace Microsoft.Data.Edm.Validation
{
	...
	{
		// Token: 0x06000C9D RID: 3229 RVA: 0x00025448 File Offset: 0x00023648
		...
		// Token: 0x06000C9E RID: 3230 RVA: 0x000254A8 File Offset: 0x000236A8
		...
		public static readonly ValidationRule<IEdmEntityContainer> EntityContainerDuplicateEntityContainerMemberName = new ValidationRule<IEdmEntityContainer>(delegate(ValidationContext context, IEdmEntityContainer entityContainer)
		{
			...
			foreach (IEdmEntityContainerElement edmEntityContainerElement in entityContainer.Elements)
			{
				...
				if (edmFunctionImport != null)
				{
					...
					if (dictionary.TryGetValue(edmFunctionImport.Name, ref list))
					{
		...
		{
			if ((entitySet.ElementType.Key() == null || Enumerable.Count<IEdmStructuralProperty>(entitySet.ElementType.Key()) == 0) && !context.IsBad(entitySet.ElementType))
			{
		...
		public static readonly ValidationRule<IEdmStructuredType> StructuredTypeInvalidMemberNameMatchesTypeName = new ValidationRule<IEdmStructuredType>(delegate(ValidationContext context, IEdmStructuredType structuredType)
		{
			...
			{
				List<IEdmProperty> list2 = Enumerable.ToList<IEdmProperty>(structuredType.Properties());
				if (list2.Count > 0)
				{
		...
		public static readonly ValidationRule<IEdmStructuredType> StructuredTypePropertyNameAlreadyDefined = new ValidationRule<IEdmStructuredType>(delegate(ValidationContext context, IEdmStructuredType structuredType)
		{
			...
			foreach (IEdmProperty edmProperty2 in structuredType.Properties())
			{
				...
				{
					ValidationHelper.AddMemberNameToHashSet(edmProperty2, hashSetInternal4, context, EdmErrorCode.AlreadyDefined, Strings.EdmModel_Validator_Semantic_PropertyNameAlreadyDefined(edmProperty2.Name), !Enumerable.Contains<IEdmProperty>(structuredType.DeclaredProperties, edmProperty2));
				}
			}
		});
		...
		{
			if ((entityType.Key() == null || Enumerable.Count<IEdmStructuralProperty>(entityType.Key()) == 0) && entityType.BaseType == null)
			{
		...
		{
			if (!Enumerable.Any<IEdmProperty>(rowType.Properties()))
			{
		...
		public static readonly ValidationRule<IEdmNavigationProperty> NavigationPropertyTypeMismatchRelationshipConstraint = new ValidationRule<IEdmNavigationProperty>(delegate(ValidationContext context, IEdmNavigationProperty navigationProperty)
		{
			...
			{
				int num = Enumerable.Count<IEdmStructuralProperty>(dependentProperties6);
				...
				if (num == Enumerable.Count<IEdmStructuralProperty>(enumerable2))
				{
					for (int i = 0; i < num; i++)
					{
						if (!Enumerable.ElementAtOrDefault<IEdmStructuralProperty>(navigationProperty.DependentProperties, i).Type.Definition.IsEquivalentTo(Enumerable.ElementAtOrDefault<IEdmStructuralProperty>(enumerable2, i).Type.Definition))
						{
							string text4 = Strings.EdmModel_Validator_Semantic_TypeMismatchRelationshipConstraint(Enumerable.ToList<IEdmStructuralProperty>(navigationProperty.DependentProperties)[i].Name, navigationProperty.DeclaringEntityType().FullName(), Enumerable.ToList<IEdmStructuralProperty>(enumerable2)[i].Name, edmEntityType2.Name, "Fred");
		...
		public static readonly ValidationRule<IEdmFunctionImport> FunctionImportEntityTypeDoesNotMatchEntitySet = new ValidationRule<IEdmFunctionImport>(delegate(ValidationContext context, IEdmFunctionImport functionImport)
		{
			if (functionImport.EntitySet != null && functionImport.ReturnType != null)
			{
				...
				if (edmTypeReference4.IsEntity())
				{
					...
					{
						List<IEdmNavigationProperty> list3 = Enumerable.ToList<IEdmNavigationProperty>(enumerable4);
						IEdmTypeReference edmTypeReference5 = ((list3.Count == 0) ? edmFunctionParameter3.Type : Enumerable.Last<IEdmNavigationProperty>(enumerable4).Type);
		...
		{
			if (functionImport.IsBindable && Enumerable.Count<IEdmFunctionParameter>(functionImport.Parameters) == 0)
			{
				...
				if (edmSchemaElement.SchemaElementKind != EdmSchemaElementKind.EntityContainer)
				{
					...
					if (function != null)
					{
						...
						else
						{
							...
							if (dictionary2.TryGetValue(text6, ref list4))
							{
								if (Enumerable.Any<IEdmFunction>(list4, (IEdmFunction existingFunction) => function.IsFunctionSignatureEquivalentTo(existingFunction)))
								{
									flag4 = true;
								}
							}
			...
			foreach (IEdmSchemaElement edmSchemaElement2 in model.SchemaElements)
			{
				...
				if (function != null)
				{
					...
					else
					{
						...
						if (dictionary3.TryGetValue(text7, ref list5))
						{
							if (Enumerable.Any<IEdmFunction>(list5, (IEdmFunction existingFunction) => function.IsFunctionSignatureEquivalentTo(existingFunction)))
							{
								flag5 = true;
							}
						}
				...
				else
				{
					...
					else
					{
						...
						else
						{
							...
							if (edmFunction != null)
							{
								flag6 = Enumerable.Any<IEdmFunction>(context.Model.FindFunctions(edmFunction.FullName()));
							}
							else
							{
								...
								{
									flag6 = Enumerable.Any<IEdmFunctionImport>(edmFunctionImport3.Container.FindFunctionImports(edmFunctionImport3.Name));
								}
								else
								{
		...
		public static readonly ValidationRule<IEdmApplyExpression> FunctionApplicationExpressionParametersMatchAppliedFunction = new ValidationRule<IEdmApplyExpression>(delegate(ValidationContext context, IEdmApplyExpression expression)
		{
			...
			{
				if (Enumerable.Count<IEdmFunctionParameter>(edmFunctionReferenceExpression.ReferencedFunction.Parameters) != Enumerable.Count<IEdmExpression>(expression.Arguments))
				{
					context.AddError(new EdmError(expression.Location(), EdmErrorCode.IncorrectNumberOfArguments, Strings.EdmModel_Validator_Semantic_IncorrectNumberOfArguments(Enumerable.Count<IEdmExpression>(expression.Arguments), edmFunctionReferenceExpression.ReferencedFunction.FullName(), Enumerable.Count<IEdmFunctionParameter>(edmFunctionReferenceExpression.ReferencedFunction.Parameters))));
				}
