...
/** ----- ResizeSensor ----- */
		...
		var ResizeSensor = function (targetElement, callback) {
			...
			if (supportsAttachEvent) {
				...
				this.triggerElements.container.attachEvent('onresize', this.boundOnResizeHandler);
				return;
			}
		...
		ResizeSensor.prototype.getResizeTrigger = function () {
		...
		ResizeSensor.prototype.insertResizeTriggerElements = function () {
			var resizeTrigger = this.getResizeTrigger();
		...
		ResizeSensor.prototype.getDimensions = function () {
			return {
				width: this.triggerElements.container.offsetWidth,
				height: this.triggerElements.container.offsetHeight
			};
		};
...
define('resizeSensor', ['ResizeSensor'],
	/**
	 * @param ResizeSensor
	...
	function (ResizeSensor) {
		...
		/** {[]} Array of all resize sensors (sensorId => ResizeSensor) */
		 ...
		 * @returns {ResizeSensor}
		 */
		ResizeSensorApi.prototype.create = function (targetElement, sensorId, callback) {
		 ...
		 * @param {number} sensorId
		 */
		ResizeSensorApi.prototype.destroy = function (sensorId) {
			/** @var ResizeSensor */
			var Sensor = allResizeSensors[sensorId];

			if (!Sensor) {
				console && console.error("Can't destroy ResizeSensor (404 not found).", sensorId);
			}
