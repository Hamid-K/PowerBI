...
namespace Microsoft.Mashup.Engine1.Library.Uris.Internal
{
	...
	{
		// Token: 0x06001C24 RID: 7204 RVA: 0x00042DC4 File Offset: 0x00040FC4
		internal static bool CheckIriUnicodeRange(char unicode, bool isQuery)
		{
			return (unicode >= '\u00a0' && unicode <= '\ud7ff') || (unicode >= '豈' && unicode <= '﷏') || (unicode >= 'ﷰ' && unicode <= '\uffef') || (isQuery && unicode >= '\ue000' && unicode <= '\uf8ff');
		}

		// Token: 0x06001C25 RID: 7205 RVA: 0x00042E1C File Offset: 0x0004101C
		...
		// Token: 0x06001C26 RID: 7206 RVA: 0x00043038 File Offset: 0x00041238
		internal static bool CheckIsReserved(char ch, UriComponents component)
		{
			...
			{
				return component == (UriComponents)0 && InternalUri.IsGenDelim(ch);
			}
		...
		// Token: 0x06001C27 RID: 7207 RVA: 0x00043134 File Offset: 0x00041334
		internal unsafe static string EscapeUnescapeIri(char* pInput, int start, int end, UriComponents component)
		{
			...
			while (i < end)
			{
				...
				if ((c = pInput[i]) == '%')
				{
					...
					c = InternalUriHelper.EscapedAscii(pInput[i + 1], pInput[i + 2]);
					if (c == '\uffff' || c == '%' || IriHelper.CheckIsReserved(c, component) || InternalUriHelper.IsNotSafeForUnescape(c))
					{
						ptr[(IntPtr)(num2++) * 2] = pInput[i++];
						ptr[(IntPtr)(num2++) * 2] = pInput[i++];
						ptr[(IntPtr)(num2++) * 2] = pInput[i];
					...
					else
					{
						...
						if (chars != 0)
						{
							InternalUriHelper.MatchUTF8Sequence(ptr, array, ref num2, array3, chars, array2, num4, component == UriComponents.Query, true);
							goto IL_02F0;
						}
				...
				else
				{
					...
					else
					{
						...
						if (!InternalUri.IsBidiControlCharacter(c) || !InternalUriParser.DontKeepUnicodeBidiFormattingCharacters)
						{
							ptr[(IntPtr)(num2++) * 2] = pInput[i];
							goto IL_02F0;
						}
				...
				if (flag2)
				{
					...
					{
						InternalUriHelper.EscapeAsciiChar((char)array6[l], array, ref num2);
					}
					array7 = null;
					goto IL_03EA;
				}
