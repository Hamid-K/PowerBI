...
namespace Microsoft.OData
{
	...
	{
		// Token: 0x06000A28 RID: 2600 RVA: 0x0001A888 File Offset: 0x00018A88
		public static Uri BuildUri(this ODataUri odataUri, ODataUrlKeyDelimiter urlKeyDelimiter)
		{
			...
			if (odataUri.Apply != null)
			{
				...
				string text3 = applyClauseToStringBuilder.TranslateApplyClause(odataUri.Apply);
			...
			if (odataUri.Index != null)
			{
				text = ODataUriExtensions.WriteQueryPrefixOrSeparator(flag, text);
				flag = false;
				text = text + "$index" + "=" + Uri.EscapeDataString(odataUri.Index.ToString());
			}
			...
			{
				string text4 = nodeToStringBuilder.TranslateParameterAliasNodes(odataUri.ParameterAliasNodes);
				text = (string.IsNullOrEmpty(text4) ? text : (ODataUriExtensions.WriteQueryPrefixOrSeparator(flag, text) + text4));
			}
			string text5 = odataUri.Path.ToResourcePathString(urlKeyDelimiter) + text;
			if (!(odataUri.ServiceRoot == null))
			{
				return new Uri(odataUri.ServiceRoot, new Uri(text5, UriKind.Relative));
			}
			return new Uri(text5, UriKind.Relative);
		}

		// Token: 0x06000A29 RID: 2601 RVA: 0x0001ABBE File Offset: 0x00018DBE
