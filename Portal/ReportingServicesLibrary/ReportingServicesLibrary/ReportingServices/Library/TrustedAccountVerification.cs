using System;
using System.Diagnostics;
using System.Security.Permissions;
using Microsoft.ReportingServices.Diagnostics.Utilities;

namespace Microsoft.ReportingServices.Library
{
	// Token: 0x020000A2 RID: 162
	[StrongNameIdentityPermission(SecurityAction.LinkDemand, PublicKey = "")]
	public static class TrustedAccountVerification
	{
		// Token: 0x0600079F RID: 1951 RVA: 0x0001FB5B File Offset: 0x0001DD5B
		public static void VerifyTrustedAccountAccess(string userName, string item)
		{
			if (RSTrace.CatalogTrace.TraceError)
			{
				RSTrace.CatalogTrace.Trace(TraceLevel.Error, "TrustedHeader Not Supported in Native Mode.");
			}
			throw new OperationNotSupportedNativeModeException();
		}
	}
}
